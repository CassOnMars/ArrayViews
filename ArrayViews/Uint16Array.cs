// <copyright file="Uint16Array.cs" company="Antidecorum">
...
namespace ArrayViews
{
    ...
    /// <summary>
    /// Describes a view over a byte array as an unsigned short array.
    /// </summary>
    public class Uint16Array : ITypedArray<ushort>
    {
        private byte[] buffer;
        ...
        public Uint16Array(byte[] buffer, int byteOffset = 0, int length = 0, bool? isLittleEndian = null)
        {
            this.buffer = buffer;
        ...
        /// <summary>
        /// Accesses the ushort at the corresponding index, adjusted for offset.
        /// and length.
        /// </summary>
        /// <param name="index">The index of the typed array.</param>
        /// <returns>A ushort at the provided index.</returns>
        public ushort this[int index]
        {
            get
            {
                if (this.Length > index && index >= 0)
                {
                    ...
                    {
                        // [0x0A,0x0B] = 0x0B0A
                        return (ushort)((this.buffer[this.byteOffset + (index * this.BytesPerElement) + 1] << 8) | this.buffer[this.byteOffset + (index * this.BytesPerElement)]);
                    }
                    else
                    {
                        // [0x0A,0x0B] = 0x0A0B
                        return (ushort)((this.buffer[this.byteOffset + (index * this.BytesPerElement)] << 8) | this.buffer[this.byteOffset + (index * this.BytesPerElement) + 1]);
                    }
                }
                else
                {
            ...
            set
            {
                if (this.Length > index && index >= 0)
                {
                    ...
                    {
                        this.buffer[this.byteOffset + (index * this.BytesPerElement)] = (byte)(value >> 8);
